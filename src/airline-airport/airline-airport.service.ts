...
export class AirlineAirportService {
    ...
    async addAirportAirline(airlineId: string, airportId: string): Promise<AirlineEntity> {
        const airport: AirportEntity = await this.airportRepository.findOne({where: {id: airportId}}); //REVISAR relations:
        if (!airport)
    ...
    async findAirportByAirlineIdArtworkId(airlineId: string, airportId: string): Promise<AirportEntity> {
        const airport: AirportEntity = await this.airportRepository.findOne({where: {id: airportId}}); //REVISAR relations:
        if (!airport)
          throw new BusinessLogicException("The airport with the given id was not found", BusinessError.NOT_FOUND)
